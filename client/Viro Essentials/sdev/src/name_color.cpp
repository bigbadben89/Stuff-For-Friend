...
namespace name_color
{
    ...
    {
        { 1, HexColor::LightBlue },  // this is range 1
        { 2, HexColor::Blue }, //this is range 2
        { 3, HexColor::Green },
        { 4, HexColor::Yellow },
        { 5, HexColor::Orange },
        { 6, HexColor::Red },
    ...
    D3DCOLOR get_helmet_name_color(CCharacter* user)
    {
        ...
        // Safety check: ensure user health > 0 for HP-based coloring
        switch (itemInfo->range)
        {
        case 92: // HP-based color green to red     my hp helm is range 92
        {
            if (user->maxHealth == 0)
                return D3DCOLOR_ARGB(255, 255, 255, 255);

            float hpPercent = static_cast<float>(user->health) / user->maxHealth;
            hpPercent = std::clamp(hpPercent, 0.0f, 1.0f);

            int r = static_cast<int>((1.0f - hpPercent) * 255);
            int g = static_cast<int>(hpPercent * 255);
            return D3DCOLOR_ARGB(255, r, g, 0);
        }
        case 99: // Smooth rainbow    this is range 99
        {
            ULONGLONG tick = GetTickCount64();
            float t = fmodf(tick / 10000.0f, 1.0f) * 2.0f * D3DX_PI;    // 10000.0f is the speed   20000.0f would be slower   8000 alot faster ect
            int r = static_cast<int>(sinf(t) * 127 + 128);
            int g = static_cast<int>(sinf(t + 2.0944f) * 127 + 128);
            int b = static_cast<int>(sinf(t + 4.1888f) * 127 + 128);
            return D3DCOLOR_ARGB(255, r, g, b);
        }
        case 93: // Rainbow + pulsing alpha
        {
            ULONGLONG tick = GetTickCount64();
            float t = fmodf(tick / 10000.0f, 1.0f) * 2.0f * D3DX_PI;
            int a = static_cast<int>((sinf(tick * 0.005f) + 1.0f) * 127.5f);
            int r = static_cast<int>(sinf(t) * 127 + 128);
            int g = static_cast<int>(sinf(t + 2.0944f) * 127 + 128);
            int b = static_cast<int>(sinf(t + 4.1888f) * 127 + 128);
        ...
        default:
            if (auto it = g_itemRangeToColor.find(itemInfo->range); it != g_itemRangeToColor.end())
                return std::to_underlying(it->second);
            break;
        }

        return D3DCOLOR_ARGB(255, 255, 255, 255); // fallback white
    }
}
