...
void Small_Icon_Save() {
	...
	{
		_asm {
			mov byte ptr ds : [small_icon_cmd] , 0x1
		}
	}
	else if (_othereff == szFalse)
	{
		_asm {
			mov byte ptr ds : [small_icon_cmd] , 0x0
		}
	}
}
...
void auraEffect_Save() {
	GetPrivateProfileStringA("VIDEO", "AURAEFFECT", "", _othereff, 255, ".\\CONFIG.ini");
	if (_othereff == szTrue)
	{
		_asm {
			mov byte ptr ds : [auraEffect] , 0x1
		}
	}
	else if (_othereff == szFalse)
	{
		_asm {
			mov byte ptr ds : [auraEffect] , 0x0
		}
	}
}
...
DWORD costume_render_retn = 0x41634D;
_declspec(naked) void costume_render()
{
	_asm {
		cmp byte ptr[Traje_Pointer], 00
		sete al
		mov byte ptr[esi + 0xAC], al
		jmp costume_render_retn
	}
}
...
void hook::GraphicOptions() {
	...
	CreateThread(NULL, NULL, LPTHREAD_START_ROUTINE(auraEffect_Save), NULL, 0, 0);
	...
	util::detour((LPVOID)0x416343, costume_render, 10);
