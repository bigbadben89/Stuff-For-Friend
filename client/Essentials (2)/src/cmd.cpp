...
void load_advanced_config()
{
    ...
    GetPrivateProfileStringA("ADVANCED", "KillFeed", "TRUE", str.data(), str.size(), g_var->iniFileName.data());
    g_showFeed = str.compare(0, 4, "TRUE") == 0 ? true : false;
...
int command_handler(char* text)
{
    ...
    if (iss.str() == "/feed on")
    {
        g_showFeed = true;
        g_showFeed = true;
        WritePrivateProfileStringA("ADVANCED", "KillFeed", "TRUE", g_var->iniFileName.data());
        return 0;
    }
    ...
    if (iss.str() == "/feed off")
    {
        g_showFeed = false;
        g_showFeed = false;
        WritePrivateProfileStringA("ADVANCED", "KillFeed", "FALSE", g_var->iniFileName.data());
        return 0;
    }
